...
one), product, order id, and their running purchase count. Also posts the
giveaway-winner announcement (see bot/services/fulfillment.py).
...
from bot.db.models import Giveaway, Order, OrderStatus, User
...
async def post_giveaway_announcement(
    bot: Bot, session, giveaway: Giveaway, winner_user_ids: list[int]
) -> None:
    """Announce a completed giveaway round's winner(s) to the public shop
    channel — called from bot/services/fulfillment.py right after the
    round closes."""
    if not config.review_channel_id:
        return
    ...
    lines = [
        ...
        f"👤 Ғолиб: {', '.join(names)}",
        "",
        "Ташаккур ба ҳама иштирокчиён! Туҳфаҳои нав дар пеш аст 🔥",
    ]
