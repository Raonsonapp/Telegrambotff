...
async def _open_catalog_message(
    ...
    if not products:
        await message.answer(
            "Ҳозир маҳсулот дастрас нест. Лутфан баъдтар кӯшиш кунед "
            "ё бо админ тамос гиред.",
            reply_markup=back_to_menu_keyboard(),
        )
        return
...
async def cart_checkout(
    ...
    total = sum(
        p.price_somoni for p in products
    )
...
async def menu_profile(callback: CallbackQuery) -> None:
    ...
    text = (
        ...
        f"💰 Маблағи умумии харид: {total:.2f} сомонӣ\n"
        f"🤝 Баланси реферал: "
        f"{user.referral_balance:.2f} сомонӣ"
    )
...
async def menu_referral(callback: CallbackQuery) -> None:
    ...
    text = (
        ...
        f"💰 Балансӣ рефералӣ: "
        f"{user.referral_balance:.2f} сомонӣ\n\n"
        "🎁 Барои ҳар дӯсте, ки тавассути линки шумо ба бот "
        "ворид шуда, харидро анҷом медиҳад "
        "(ва он аз ҷониби админ тасдиқ мешавад), шумо 5% "
        "аз маблағи хариди ӯро ҳамчун бонус мегиред.\n\n"
        "💳 Бонуси ҷамъшуда ба балансии шумо илова мешавад ва "
        "метавонед онро барои пардохти харидҳо дар бот истифода баред."
    )
...
async def _finalize_recipient(
    ...
    for p in products:
        ...
        confirm_lines.append(
            f"🎁 Маҳсулот: "
            f"{p.diamonds}{bonus}{p.unit_label}"
        )
...
async def pay_with_balance(
    ...
    if config.admin_chat_id:
        await callback.bot.send_message(
            ...
            f"👤 Мизоҷ: {callback.from_user.full_name} "
...
async def confirm_order(
    ...
    # 🔥 НОВОЕ: ба ҷойи як тугмаи Pay ҳоло 3 роҳи пардохт
...
async def payment_dc(
    ...
    text = (
        f"🏙️ <b>ДУШАНБЕ СИТИ</b>\n\n"
        f"💳 Рақами корти қабулкунанда:\n"
        f"<code>9762000199761387</code>\n\n"
        f"💰 Маблағи пардохт:\n"
        f"<code>{amount:.2f}</code> сомонӣ\n\n"
        f"⚠️ Маблағро айнан "
        f"<b>{amount:.2f}</b> сомонӣ гузаронед.\n"
        f"На кам ва на зиёд.\n\n"
        f"📸 Пас аз пардохт скриншоти расидро "
        f"ба ҳамин чат фиристед."
    )
...
async def payment_alif(
    ...
    text = (
        f"🟢 <b>ALIF</b>\n\n"
        f"📱 Рақами қабулкунанда:\n"
        f"<code>976820008</code>\n\n"
        f"💰 Маблағи пардохт:\n"
        f"<code>{amount:.2f}</code> сомонӣ\n\n"
        f"⚠️ Маблағро айнан "
        f"<b>{amount:.2f}</b> сомонӣ гузаронед.\n"
...
@router.callback_query(
    OrderFlow.awaiting_payment_proof,
    F.data == "payment:eskhata",
)
async def payment_eskhata(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    data = await state.get_data()
    ...
    async with get_session() as session:
        ...
        if len(group) > 1:
            ...
            items_summary = "\n" + "\n".join(
                ...
                if o.amount_somoni
                else f"📦 {p.diamonds}{p.unit_label}"
                for o, p in zip(group, products)
            )

    caption = (
        f"🆕 Фармоиши #{order_id}{items_summary}\n"
        f"👤 Мизоҷ: {message.from_user.full_name} "
        f"(@{message.from_user.username or '—'}, "
        f"id={message.from_user.id})\n"
        f"Расиди пардохт замима шуд.\n\n"
        f"❗️ Пеш аз тасдиқ, ҳатман дар аппи бонки худ "
        f"маблағи воқеиро санҷед — расм танҳо кофӣ нест."
    )
